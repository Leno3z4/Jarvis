...
function rejectionSummary(discovered: number, scannerCandidates: number, opportunities: AutomationResult["opportunities"]): string {
  if (opportunities.length === 0) return `No strategy candidates passed the deterministic scanner (discovered ${discovered} markets; scanner candidates ${scannerCandidates}).`;
  ...
  return `No executable opportunity found (discovered ${discovered}; scanner candidates ${scannerCandidates}). ${top}`;
}
...
export async function evaluateAutomation(config: AutomationConfig): Promise<AutomationResult> {
  ...
  if (!best) return { discovered: result.discovered, scannerCandidates: result.scannerCandidates, opportunities: result.opportunities, blockedReason: rejectionSummary(result.discovered, result.scannerCandidates, result.opportunities) };
  ...
  if (gateError) return { discovered: result.discovered, scannerCandidates: result.scannerCandidates, opportunities: result.opportunities, blockedReason: gateError };
  ...
  return { discovered: result.discovered, scannerCandidates: result.scannerCandidates, opportunities: result.opportunities, trade };
}

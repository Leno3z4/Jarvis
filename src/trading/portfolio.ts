...
export function toPortfolio(state: PortfolioState): Portfolio {
  return { cashWei: state.cashWei, positions: { ...state.positions }, costBasisWei: { ...(state.costBasisWei ?? {}) }, realizedPnlWei: state.realizedPnlWei };
}
...
export function applyPaperFill(state: PortfolioState, request: TradeRequest, amountOutWei: bigint): PortfolioState {
  ...
  const inputKey = key(request.tokenIn), outputKey = key(request.tokenOut), cashKey = key(state.cashToken);
  ...
  const next: PortfolioState = { ...state, positions: { ...state.positions }, costBasisWei: { ...(state.costBasisWei ?? {}) } };
  ...
  if (next.positions[inputKey] === 0n) { delete next.positions[inputKey]; delete next.costBasisWei![inputKey]; }

  if (outputKey === cashKey) next.cashWei += amountOutWei, next.realizedPnlWei += amountOutWei - positionCost;
  else {
    next.positions[outputKey] = (next.positions[outputKey] ?? 0n) + amountOutWei;
    next.costBasisWei![outputKey] = (next.costBasisWei![outputKey] ?? 0n) + request.amountInWei;
  }

...
export function getConfig(env: Env): JarvisConfig {
  ...
  return {
    ...
    gemini: {
      ...
      fallback1Model: env.GEMINI_MODEL_FALLBACK_1 ?? "gemini-2.5-flash",
    ...
    strategy: {
      ...
      maxCandidates: Math.max(1, Math.floor(numberEnv(env.STRATEGY_MAX_CANDIDATES, 5))),
      ...
      slippageBps: Math.max(1, Math.min(500, Math.floor(numberEnv(env.STRATEGY_SLIPPAGE_BPS, 100))))
    }
  };
}

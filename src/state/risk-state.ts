...
function defaultState(): RiskStateData {
  return { killSwitch: false, dayKey: dayKeyUtc(), dailyLossWei: 0n, dailyTrades: 0, lastTradeAtByToken: {} };
}
...
function normalizeDay(state: RiskStateData, nowMs: number): RiskStateData {
  const dayKey = dayKeyUtc(new Date(nowMs));
  return state.dayKey === dayKey ? state : { ...defaultState(), dayKey, killSwitch: state.killSwitch };
}
...
export class RiskState {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    let state = normalizeDay(readState(this.sql, nowMs), nowMs);

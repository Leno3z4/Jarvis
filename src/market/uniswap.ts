...
const V4_FEE_TIERS = [
  ...
  { fee: 3000, tickSpacing: 60 },
  { fee: 10000, tickSpacing: 200 },
  { fee: 3000, tickSpacing: 10 },
  { fee: 500, tickSpacing: 60 }
] as const;
...
async function poolInfo(apiKey: string, token: `0x${string}`, quoteTokenAddress: `0x${string}`, protocol: (typeof POOL_PROTOCOLS)[number], fee?: number, tickSpacing?: number): Promise<PoolInfo[]> {
  ...
  if (protocol === "V3") {
    if (fee === undefined || tickSpacing === undefined) throw new Error("V3 pool lookup requires fee and tickSpacing.");
...
function marketFromPool(token: UniswapToken, pool: PoolInfo, quoteTokenAddress: `0x${string}`, quotePriceUsd: number): TokenMarket | null {
  ...
  if (!Number.isFinite(tokenDecimals) || !Number.isFinite(quoteDecimals)) return null;
  ...
  const protocol = pool.poolProtocol?.toUpperCase();
  let priceUsd = 0;
  let liquidityUsd = 0;

  if (protocol === "V3" && pool.sqrtRatioX96 && pool.poolLiquidity) {
    const sqrtPrice = Number(pool.sqrtRatioX96) / Q96;
    const liquidity = positiveNumber(pool.poolLiquidity);
    ...
    const rawVirtual0 = liquidity / sqrtPrice;
    const rawVirtual1 = liquidity * sqrtPrice;
    ...
    const tokenAmount = tokenIsA ? amountA : amountB;
    const quoteAmount = tokenIsA ? amountB : amountA;
    const tokenValueUsd = tokenAmount * priceUsd;
    const quoteValueUsd = quoteAmount * quoteUsd;
    liquidityUsd = 2 * Math.min(tokenValueUsd, quoteValueUsd);
  } else {
    ...
    priceUsd = (quoteAmount / tokenAmount) * quoteUsd;
    liquidityUsd = quoteAmount * quoteUsd * 2;
  }
  ...
  async discoverBaseMarkets(limit = 5): Promise<TokenMarket[]> {
    ...
    const tryPool = async (token: UniswapToken, quoteToken: `0x${string}`): Promise<TokenMarket[]> => {
      ...
      for (const protocol of ["V2", "V3"] as const) {
        if (protocol === "V2") {
          try {
            ...
            failures.push(`${token.symbol}:V2:${quoteToken === BASE_USDC ? "USDC" : "WETH"}:${error instanceof Error ? error.message : "unknown"}`);
          }
        } else {
      ...
      for (const feeTier of V4_FEE_TIERS) {
        try {
          const pools = await poolInfo(this.apiKey, token.address as `0x${string}`, quoteToken, "V4", feeTier.fee, feeTier.tickSpacing);
          for (const pool of pools) {
            const market = marketFromPool(token, pool, quoteToken, wethPriceUsd);
            if (market) candidates.push(market);
          }
        } catch (error) {
          failures.push(`${token.symbol}:V4:${quoteToken === BASE_USDC ? "USDC" : "WETH"}:${feeTier.fee}/${feeTier.tickSpacing}:${error instanceof Error ? error.message : "unknown"}`);
        }
      }
      return candidates;
    };
    ...
    for (const token of tokens) {
      ...
      else failures.push(`${token.symbol}: no usable V2/V3/V4 pool for USDC or WETH`);
    }

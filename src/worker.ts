...
function json(body: unknown, init: ResponseInit = {}): Response { const headers = new Headers(init.headers); for (const [k, v] of Object.entries(CORS)) headers.set(k, v); return Response.json(body, { ...init, headers }); }
...
async function settleRisk(env: Env, beforePnl: bigint, config: ReturnType<typeof getConfig>) {
  const after = await getPaperPortfolio(env, config);
  const delta = BigInt(after.realizedPnlWei) - beforePnl;
  await riskStub(env).fetch(new Request("https://jarvis-risk/settle", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ realizedPnlDeltaWei: delta.toString() }) }));
}
async function executePaperTrade(env: Env, config: ReturnType<typeof getConfig>, trade: TradeRequest) {
  const before = await getPaperPortfolio(env, config);
  const response = await stateStub(env).fetch(new Request(`https://jarvis.internal/paper/trade?cashToken=${config.paperCashToken}&startingCashWei=${config.paperStartingCashWei}`, {
    method: "POST", body: JSON.stringify(trade, (_, value) => typeof value === "bigint" ? value.toString() : value), headers: { "content-type": "application/json" }
  }));
  if (response.ok) await settleRisk(env, BigInt(before.realizedPnlWei), config);
  return response;
}
async function runPaperCycle(env: Env, config: ReturnType<typeof getConfig>) {
  if (config.mode !== "paper" || !config.zeroExApiKey || !config.paperTakerAddress || !config.gemini.primaryKey) return { executed: false, reason: "Paper automation is not fully configured." };
  const result = await evaluateAutomation({
    ...
    strategy: { ...config.strategy, cashToken: config.paperCashToken }, zeroExApiKey: config.zeroExApiKey, takerAddress: config.paperTakerAddress,
    cashToken: config.paperCashToken, quoteAmountWei: config.strategy.quoteAmountWei, slippageBps: config.strategy.slippageBps, risk: config.risk
  });
  ...
  const response = await executePaperTrade(env, config, result.trade);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const url = new URL(request.url); const config = getConfig(env);
    ...
    if (url.pathname === "/strategy/run" && request.method === "POST") {
      try { return json({ ok: true, ...(await runPaperCycle(env, config)) }); }
    ...
    if (url.pathname === "/trade" && request.method === "POST") {
      try {
        ...
        const payload = await request.json() as TradePayload; const trade = parseTrade(payload); const basic = validateTrade(trade, config.risk);
        if (basic) return json({ ok: false, error: basic }, { status: 400 });
        const riskResponse = await authorizePaperTrade(env, config, trade); if (riskResponse) return riskResponse;
        const before = await getPaperPortfolio(env, config);
        const forwarded = new Request(request.url, { method: request.method, headers: request.headers, body: JSON.stringify({ ...payload, amountInWei: trade.amountInWei.toString(), amountOutWei: trade.amountOutWei.toString() }) });
        const response = await legacy.fetch(forwarded, env);
        if (response.ok) { const after = await getPaperPortfolio(env, config); const delta = BigInt(after.realizedPnlWei) - BigInt(before.realizedPnlWei); await riskStub(env).fetch(new Request("https://jarvis-risk/settle", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ realizedPnlDeltaWei: delta.toString() }) })); }
        return response;
  ...
  async scheduled(_event: ScheduledEvent, env: Env): Promise<void> {
    const config = getConfig(env); if (config.mode === "paper") await runPaperCycle(env, config);
